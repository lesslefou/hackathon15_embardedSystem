...
int connexion(char * IP,int port)
{
	...
	if(!erreur)
	{
		...
		if(sock != -1)
		{
			...
			my_log_function(INFO,bufferLog,"connexion");
			driveClock(DEFAULT_TIME_CONNECTION);
...
int driveClock(int time)
{
	...
	{
		begin = clock();
		retard = time*CLOCKS_PER_SEC;
		my_log_function(INFO,"Début de la clock","driveClock");
	}
	else if(begin+clock() >  retard)
	{
...
char * createHeader(char * cible,int type,int create)
{
	...
	switch(type)
	{
		...
		case CREATE: Type = "POST"; Create[1] = create%10; Create[0] = (create-Create[1])/10; Create[2] = '\0'; break;
...
X-M2M-Origin: %s\r\n\
X-M2M-RI: %i\r\n\
X-M2M-RVI: %s\r\n",Type,cible,DEFAULT_HTTP_VERSION,DEFAULT_USER_AGENT,Create,CONTENT_TYPE,CONTENT_TYPE,DEFAULT_X_M2M_ORIGIN,requestNumber,DEFAULT_X_M2M_RVI);
	 requestNumber++;
